  ...
  public void configure(AuthenticationManagerBuilder auth) throws Exception {
      auth
        .jdbcAuthentication()
          ...
          .authoritiesByUsernameQuery("select t1.username, t2.authority from (select u.username as username, c.nome as credencial from usuario u, usuario_credencial uc, credencial c where u.id = uc.fk_usuario and c.id = uc.fk_credencial) t1 inner join (select c.nome as credencial, a.nome as authority from credencial c, credencial_autorizacoes ca, autorizacao a where c.id = ca.credencial_id and a.id = ca.autorizacoes_id) t2 on t1.credencial = t2.credencial where t1.username =  ?")
